...
1. [Architecture](./architecture.md)
   - `apps/web`, `apps/admin`, `apps/api`, `packages/shared`의 역할과 모노레포 구조
   - public domain, Studio, Admin, legacy authoring API의 경계
...
3. [Features](./features.md)
   - public feed/channel/viewer/community
   - Studio 프로젝트, 에피소드 에디터, publish, analytics, asset/member 관리
   - Admin 콘솔과 Discourse 연동 흐름

4. [Maintenance](./maintenance.md)
   - 로컬 DB, 환경 변수, 마이그레이션, 개발 서버 실행
   - 테스트/빌드 명령
   - projection repair, auth/session, upload, Discourse 관련 운영 메모
