import type { ExperimentalAccessSummaryResponse, FeedHomeResponse, FeedItem, FeedResponse } from '@promptoon/shared';
...
import { ConsumerExperimentalPage } from '../src/domains/consumer/pages/ConsumerExperimentalPage';

const getHomeMock = vi.hoisted(() => vi.fn());
const getBookmarksMock = vi.hoisted(() => vi.fn());
const unbookmarkPublishMock = vi.hoisted(() => vi.fn());
const getMyExperimentalAccessMock = vi.hoisted(() => vi.fn());
const getExperimentalFeedMock = vi.hoisted(() => vi.fn());
...
vi.mock('../src/shared/api/experimental.api', () => ({
  experimentalApi: {
    getMyAccess: getMyExperimentalAccessMock,
    getMyFeed: getExperimentalFeedMock,
...
beforeEach(() => {
  ...
  getMyExperimentalAccessMock.mockReset();
  getExperimentalFeedMock.mockReset();
  redeemExperimentalInviteCodeMock.mockReset();
  getMyExperimentalAccessMock.mockResolvedValue({ grantCount: 0, grants: [] } satisfies ExperimentalAccessSummaryResponse);
  getExperimentalFeedMock.mockResolvedValue({ items: [] });
  redeemExperimentalInviteCodeMock.mockResolvedValue({
    ...
    target: {
      ...
      targetType: 'publish',
      projectId: 'project-1',
      projectTitle: 'Experimental Project',
      publishId: 'publish-experimental',
      episodeId: 'episode-experimental',
      episodeTitle: 'Experimental Episode',
      status: 'active',
      grantCount: 1,
      inviteCodeCount: 1,
...
describe('consumer mobile frame', () => {
  it('renders the consumer home with 4-tab navigation', async () => {
    const item = createFeedItem({ episodeTitle: 'Home Hero Episode', isExperimental: true });
    ...
    const { container } = renderWithProviders('/', <ConsumerHomePage />);

    expect((await screen.findAllByText('Home Hero Episode')).length).toBeGreaterThan(0);
    const firstSilhouette = screen.getAllByTestId('consumer-card-image-silhouette')[0];
    const firstPoster = container.querySelector<HTMLImageElement>('img[src="https://cdn.example.com/cover.jpg"]');

    expect(firstPoster?.getAttribute('loading')).toBe('eager');
    expect(firstPoster?.getAttribute('fetchpriority')).toBe('high');
    expect(firstSilhouette.getAttribute('data-loading-state')).toBe('loading');

    fireEvent.load(firstPoster as HTMLImageElement);

    await waitFor(() => {
      expect(firstSilhouette.getAttribute('data-loading-state')).toBe('ready');
    });
    ...
    expect(screen.getAllByText('실험용').length).toBeGreaterThan(0);
  ...
  it('renders my page for guest and authenticated users', async () => {
    ...
    useAuthStore.setState({
      user: {
        id: 'user-1',
        loginId: 'viewer0001',
        snsProfileImageUrl: 'https://cdn.example.com/google-profile.png'
      },
      session: null,
    ...
    const { container } = renderWithProviders('/my', <ConsumerMyPage />);
    expect(screen.getByText('viewer0001')).toBeTruthy();
    expect(container.querySelector('img[src="https://cdn.example.com/google-profile.png"]')).toBeTruthy();
    await waitFor(() => {
      expect(getMyExperimentalAccessMock).toHaveBeenCalledTimes(1);
    });
    expect(screen.getByText('접근 가능')).toBeTruthy();
    expect(screen.getByText('권한 필요')).toBeTruthy();
  });

  it('redeems experimental invite codes and lists accessible content', async () => {
    const user = userEvent.setup();
    useAuthStore.setState({
      user: { id: 'user-1', loginId: 'viewer0001' },
      session: null,
    ...
    getMyExperimentalAccessMock.mockResolvedValue({ grantCount: 1, grants: [] } satisfies ExperimentalAccessSummaryResponse);
    getExperimentalFeedMock.mockResolvedValue({
      items: [createFeedItem({ publishId: 'publish-experimental', episodeTitle: 'Experimental Episode', isExperimental: true })]
    });

    renderWithProviders('/experimental', <ConsumerExperimentalPage />);

    expect(await screen.findByText('Experimental Episode')).toBeTruthy();
    expect(screen.getByText('실험용')).toBeTruthy();
    await user.type(screen.getByLabelText('초대 코드'), 'ABCD-EFGH-JK23-4567');
    await user.click(screen.getByRole('button', { name: '등록' }));

    await waitFor(() => {
      expect(redeemExperimentalInviteCodeMock.mock.calls[0]?.[0]).toBe('ABCD-EFGH-JK23-4567');
    });
    expect(await screen.findByText('실험형 콘텐츠 권한이 추가되었습니다.')).toBeTruthy();
  });
});
