...
export function MovingtoonVideoPlayer({
  ...
  const syncProgress = useCallback(() => {
    ...
    const nextProgress = duration ? clampProgress((video.currentTime / duration) * 100) : 0;
    setProgress((currentProgress) => (Math.abs(currentProgress - nextProgress) < 0.25 ? currentProgress : nextProgress));
  }, []);
  ...
  function handleSeek(event: ChangeEvent<HTMLInputElement>) {
    const nextProgress = clampProgress(Number(event.currentTarget.value));
    const video = videoRef.current;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute left-4 top-4 z-30 inline-flex h-11 min-w-11 items-center justify-center gap-2 rounded-full border border-white/18 bg-black/54 px-3 text-white shadow-lg shadow-black/30 backdrop-blur-xl transition duration-200 hover:bg-black/70 focus:outline-none focus:ring-2 focus:ring-white/80 sm:pointer-events-none sm:translate-y-1 sm:opacity-0 sm:group-hover:pointer-events-auto sm:group-hover:translate-y-0 sm:group-hover:opacity-100 sm:group-focus-within:pointer-events-auto sm:group-focus-within:translate-y-0 sm:group-focus-within:opacity-100"
        onClick={(event) => {
      ...
      <button
        ...
        className={[
          'absolute left-1/2 top-1/2 z-30 inline-flex h-24 w-24 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full border border-white/18 bg-black/46 text-white shadow-2xl shadow-black/35 backdrop-blur-xl transition duration-200 focus:outline-none focus:ring-2 focus:ring-white/80 motion-reduce:transition-none',
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30">
        <label
          className="pointer-events-auto block h-6 w-full cursor-pointer"
          onClick={(event) => {
            event.stopPropagation();
          }}
        >
          <span className="relative block h-full w-full">
            <span className="absolute inset-x-0 bottom-0 h-1 bg-white/18" />
            <span className="absolute bottom-0 left-0 h-1 bg-white transition-[width] duration-100" style={{ width: `${progress}%` }} />
            <input
              aria-label="영상 진행 위치"
